...
from config import (BATCH_SIZE, CLASS_NUM, REPEAT_NUM, MOD_NUM, LOG_PATH, MIN_CONNECT_TUMOR_NUM, WHOLE_REPEAT_NUM,
                    MODEL_PATH, SIZE, SUMMARY_INTERVAL, VAL_BATCH_SIZE, VAL_INTERVAL, MIN_TUMOR_NUM)
...
class Net(object):
    ...
    def whole_predict(self, tfr_name, batch_size=BATCH_SIZE, with_label=True, final_result=False,
        ...
        with self.graph.as_default():
            ...
            if final_result:
                result = 0
                tfr_tumor_predict_pos = np.where(tfr_predict == 0)[0]
    ...
    def validate(self, file_list, batch_size=BATCH_SIZE, test=False):
        with self.graph.as_default():
            ...
            self.validate_report(predict_list, label_list, test, tumor_as_1=False)
